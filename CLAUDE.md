...
## Rhyffle Dev Bot 송신 / 응답 룰

- **송신 허용 채널 (유일)**: 채널 ID `1508031595565678622`. 이 채널 외 다른 어떤 Discord 채널/스레드/DM 으로도 메세지 전송 금지. 사용자가 다른 채널 ID 명시한 경우만 예외.
- **세션 시작 시 체크**: 새 세션 시작 시 `discord_read_messages` 로 위 채널 신규 메세지 확인 → 신규 메세지가 있으면 아래 "수신 시 처리" 순서대로 진행.
- **답장 자동화 한계**: Claude 는 백그라운드 데몬이 아니므로 진짜 "메세지 도착 즉시" 자동 답장은 불가. 세션 시작 시 체크가 가장 즉시성 있는 방식. 더 빠른 폴링이 필요하면 사용자가 `/loop` 또는 cron 별도 셋업.
- **수신 시 처리 순서 (항상)**:
  1. **저장**: 받은 메세지를 `.claude/context/DISCORD_LOG.md` 에 기존 형식대로 append (날짜 / 화자 / 원문 / 맥락 / spec 영향 / 상태)
  2. **스펙 반영**: 메세지 내용이 spec/계획에 영향 주면 즉시 관련 파일 갱신 — `CURRENT_STATUS.md` (블로커·펜딩·다음 액션) / `SPEC.md` (정본 표) / `DECISIONS.md` (큰 결정 새 entry) / `GLOSSARY.md` (용어/매커닉 변경) 중 해당 파일
  3. **감사 답장 (조건부)**: 받은 메세지가 **봇이 직전에 보낸 질문/요청에 대한 답변이라고 판단될 때만** `감사합니다!` 송신. 단순 안내·정보 공유·잡담 등 봇 질의와 무관한 메세지는 답장하지 않음. 판단 애매하면 답장 안 함 + 사용자에게 "이거 답장할까요?" 한 줄 확인.
  4. **보고**: 사용자에게 "메세지 저장 + 갱신 파일 N개 / 핵심 변경 요약 / 답장 여부" 한 줄
  - 잡담/감사 인사 등 spec 무관 메세지는 저장도 폐기 (CONTENT FILTER 동일 적용)
