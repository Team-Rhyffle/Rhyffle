...
### v2는 다시 두 버전 — 데모 = v2 2차 only 확정 (5/24)

| 버전 | 생성 | 핵심 변화 | 페이지 ID prefix |
|---|---|---|---|
| **v2 2차 (COSMO 통합)** ★ 데모 정본 | 2026-05-11 | 8 라인, 40장 덱, COSMO Objekt 카드 | `35d36a63...` |
| **v2 1차 (포커 기반)** ❌ 데모 미포함 | 2025-09-07 | 7 라인, 56장 덱 (52+4 조커), 포커 매커닉 | `26736a63...` |

**5/24 한울 확정**: v2 1차 매커닉(족보/카드군/양면+반물질/고유·일반능력/포커카드)은 **데모에 일절 미포함**. 카드 데이터 = Objekt 값 그대로. 향후 프로덕션 단계에서 부활 가능성 잔존하므로 코드/문서는 보존.

| 영역 | v2 1차 | v2 2차 | 데모 정본 |
|---|---|---|---|
| 라인 수 | 7 | **8** | v2 2차 ★ |
| 덱 크기 | 56장 (포커) | **40장** | v2 2차 ★ |
| 카드 데이터 | 포커 (4문양 13랭크) | **COSMO Objekt** | v2 2차 ★ |
| 점수 시스템 | 랭크 + 족보 점수 | **클래스 + 시너지 점수** | v2 2차 ★ |
| 강화 | (없음) | **화음 / 공명 (각 10단계)** | v2 2차 ★ |
| 족보 12종 + 카드군 12종 | 명시 | (없음) | ❌ **데모 미포함** (5/24) |
| 양면 카드 / 고유·일반능력 | 명시 | (없음) | ❌ **데모 미포함** (5/24) |
| 반물질 재화 | 명시 | (없음) | ❌ **데모 미포함** (5/24) |
| 포커 매커닉 (4문양 13랭크) | 명시 | (없음) | ❌ **데모 미포함** (5/24) |
